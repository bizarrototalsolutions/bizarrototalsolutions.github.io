...
const ServicoPage = {
  ...
  // Fase 6: Zona passa a ser uma referência real (zonaId), não texto
  // livre — tal como Categoria/Tipo já eram. Zonas Inativas continuam
  // a aparecer (assinaladas) para não esconder a escolha de um
  // serviço antigo já associado a uma zona entretanto desativada.
  populateZonas() {
    const select = document.getElementById('servico-zona');
  ...
  async sincronizarComCliente() {
    ...
    if (!ok) return;
    document.getElementById('servico-zona').value = cliente.zonaId || '';
  ...
  carregarParaEdicao() {
    ...
    document.getElementById('servico-zona').value = servico.zonaId || '';
  ...
  guardar(e) {
    ...
    const cliente = DB.getCliente(clienteId);
    const zonaSelect = document.getElementById('servico-zona');
    const zonaId = zonaSelect.value || null;
    // Retrato histórico do nome, tal como já fazemos com clienteNome —
    // resolvido UMA vez aqui, gravado, e nunca mais recalculado.
    const zonaNome = zonaId ? (zonaSelect.selectedOptions[0].textContent || '').replace(' (Inativa)', '').trim() : '';
