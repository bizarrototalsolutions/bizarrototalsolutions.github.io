...
const ClientesPage = {
  ...
  state: {
    ...
    sortDir: 'asc',
    search: '',
    filters: { zonaId: '', distrito: '', estado: '', codigoPostal: '' }
  },
  ...
  populateZonaFilter() {
    ...
    select.innerHTML = '<option value="">Todas as zonas</option>' +
      zonas.map(z => `<option value="${z.id}">${Utils.escapeHtml(z.nome)}</option>`).join('');
  ...
  bindEvents() {
    ...
    ['filtro-zona', 'filtro-distrito', 'filtro-estado', 'filtro-cp'].forEach(id => {
      document.getElementById(id).addEventListener('input', () => {
        this.state.filters = {
          zonaId: document.getElementById('filtro-zona').value,
    ...
    document.getElementById('limpar-filtros').addEventListener('click', () => {
      this.state.search = '';
      this.state.filters = { zonaId: '', distrito: '', estado: '', codigoPostal: '' };
  ...
  render() {
    ...
    // Cliente é dado "vivo": o nome da zona é sempre resolvido a
    // partir do zonaId no momento de mostrar, nunca guardado como
    // texto — se a zona for renomeada, a tabela reflete-o de imediato.
    const mapaZonas = this.construirMapaZonas();

    tbody.innerHTML = items.map(c => `
      <tr>
        <td><a href="cliente.html?id=${c.id}"><strong>${Utils.escapeHtml(c.nome)}</strong></a></td>
        <td>${Utils.escapeHtml(c.nif || '—')}</td>
        <td>${Utils.escapeHtml(mapaZonas[c.zonaId] || '—')}</td>
  ...
  // Um único sítio a construir o mapa zonaId->nome — reutilizado
  // pela tabela e pelas duas exportações, nunca reconstruído 3 vezes.
  construirMapaZonas() {
    const mapa = {};
    DB.getZonas().forEach(z => { mapa[z.id] = z.nome; });
    return mapa;
  },
  ...
  exportExcel() {
    ...
    const rows = items.map(c => ({
      ...
      Localidade: c.localidade, Distrito: c.distrito, Zona: mapaZonas[c.zonaId] || '',
  ...
  exportPDF() {
    ...
    const rows = items.map(c => [c.nome, c.nif || '', mapaZonas[c.zonaId] || '', c.localidade || '', c.telefone || c.telemovel || '', c.estado]);
