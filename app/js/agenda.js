...
const AgendaPage = {
  ...
  bindEvents() {
    ...
    idsFiltro.forEach(id => {
      document.getElementById(id).addEventListener('input', () => {
        this.filtros = {
          ...
          zona: document.getElementById('filtro-zona-agenda').value,
    ...
    document.getElementById('limpar-filtros-agenda').addEventListener('click', () => {
      this.filtros = { clienteId: '', estado: '', categoria: '', tipoServico: '', prioridade: '', zona: '', funcionario: '' };
      this.pesquisa = '';
  ...
  ligarAtualizacoesAutomaticas() {
    const atualizar = () => this.calendar.refetchEvents();
    DB.on('servico:criado', atualizar);
    DB.on('servico:editado', atualizar);
    DB.on('servico:estadoAlterado', atualizar);
    DB.on('servico:eliminado', atualizar);
  }
};
